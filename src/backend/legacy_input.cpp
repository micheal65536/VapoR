...
bool LegacyInputHelper::compareStatesAndEmitChangeEvents(EventQueue& eventQueue, const PoseSet& eventControllerPose, int eventControllerIndex, const LegacyInputState& oldState, const LegacyInputState& newState)
{
    bool changed = false;
    for (int buttonIndex = 0; buttonIndex < buttonsCount; buttonIndex++)
    {
        bool currentFrameTouched = getButtonState(newState.touch, buttonIndex);
        bool currentFramePressed = getButtonState(newState.click, buttonIndex);
        bool previousFrameTouched = getButtonState(oldState.touch, buttonIndex);
        bool previousFramePressed = getButtonState(oldState.click, buttonIndex);
        ...
        openvr::Event_0_9_15 event {
            ...
            .trackedDeviceIndex = (uint32_t) eventControllerIndex + 1,
            .ageSeconds = 0.0f,
            .data {
                .button {
        ...
        if (currentFrameTouched && !previousFrameTouched)
        {
            ...
            eventQueue.putEvent(event, eventControllerPose);
            changed = true;
        }
