...
static int findComponent(const std::string& subpath, OpenVRInputType type, const std::vector<Mode::ComponentDescription>& availableComponents)
{
    for (int i = 0; i < availableComponents.size(); i++)
    {
        if (availableComponents[i].subpath == subpath && (type == OpenVRInputType::NONE || availableComponents[i].type == type))
        {
            return i;
        }
    }
...
bool Mode::ClickFilter::tryGrab(Mode::ClickFilter::Source source, Mode::ClickFilter::DefaultThresholdsType defaultThresholdsType, std::optional<float> activateThresholdOverride, std::optional<float> deactivateThresholdOverride, const std::vector<ComponentDescription>& availableComponents, std::vector<int>& grabbedComponents)
{
    ...
    {
        case Source::BASE:
            if ((index1 = findComponent("", OpenVRInputType::NONE, availableComponents)) == -1)
            {
                return false;
            }
            break;
        case Source::CLICK:
            if ((index1 = findComponent("click", OpenVRInputType::BOOLEAN, availableComponents)) == -1)
            {
                return false;
            }
            break;
        case Source::TOUCH:
            if ((index1 = findComponent("touch", OpenVRInputType::BOOLEAN, availableComponents)) == -1)
            {
                return false;
            }
            break;
        case Source::VALUE:
            if ((index1 = findComponent("value", OpenVRInputType::FLOAT, availableComponents)) == -1)
            {
                return false;
            }
            ...
            break;
        case Source::FORCE:
            if ((index1 = findComponent("force", OpenVRInputType::FLOAT, availableComponents)) == -1)
            {
                return false;
            }
            convertFromScalar = true;
        ...
        case Source::POSITION:
            if ((index1 = findComponent("x", OpenVRInputType::FLOAT, availableComponents)) == -1 || (index2 = findComponent("y", OpenVRInputType::FLOAT, availableComponents)) == -1)
            {
                return false;
            }
...
bool Mode::LatchingClickFilter::tryGrab(Mode::ClickFilter::Source source, Mode::ClickFilter::DefaultThresholdsType defaultThresholdsType, std::optional<float> activateThresholdOverride, std::optional<float> deactivateThresholdOverride, bool touchy, bool sticky, std::optional<float> stickyActivateThresholdOverride, std::optional<float> stickyDeactivateThresholdOverride, const std::vector<Mode::ComponentDescription>& availableComponents, std::vector<int>& grabbedComponents)
{
    ...
    if (touchy)
    {
        int index = findComponent("touch", OpenVRInputType::BOOLEAN, availableComponents);
        if (index != -1)
        {
    ...
    if (sticky)
    {
        int index1 = findComponent("x", OpenVRInputType::FLOAT, availableComponents);
        int index2 = findComponent("y", OpenVRInputType::FLOAT, availableComponents);
        grabbedComponents.push_back(index1);
        grabbedComponents.push_back(index2);
...
std::vector<int> ButtonMode::connect(const std::vector<Mode::ComponentDescription>& availableComponents, const bool* connectedOutputs)
{
    ...
    if (clickConnectedOutput)
    {
        if (advancedOutput)
        {
            if (findComponent(button.forceInput == ForceInput::TOUCH ? "touch" : "click", OpenVRInputType::BOOLEAN, availableComponents) != -1)
            {
...
std::vector<int> TriggerMode::connect(const std::vector<Mode::ComponentDescription>& availableComponents, const bool* connectedOutputs)
{
    ...
    {
        int index = findComponent("value", OpenVRInputType::FLOAT, availableComponents);
        if (index == -1)
        {
            index = findComponent("", OpenVRInputType::FLOAT, availableComponents);
        }
    ...
    if (connectedOutputs[1])
    {
        switch (button.forceInput)
        {
            ...
            case ForceInput::TOUCH:
                if (button.tryGrabClick(ClickFilter::Source::BASE, availableComponents, indexes)) break;
                if (findComponent("click", OpenVRInputType::BOOLEAN, availableComponents) != -1 && button.tryGrabClick(ClickFilter::Source::TOUCH, availableComponents, indexes)) break;
                if (button.tryGrabClick(ClickFilter::Source::CLICK, availableComponents, indexes)) break;
                if (button.tryGrabClick(ClickFilter::Source::VALUE, availableComponents, indexes)) break;
                if (button.tryGrabClick(ClickFilter::Source::FORCE, availableComponents, indexes)) break;
                if (button.tryGrabClick(ClickFilter::Source::TOUCH, availableComponents, indexes)) break;
...
DpadMode::DpadMode(const nlohmann::json::object_t& parameters, OpenVRProfileInputType profileInputType)
{
    ...
    isJoystick = profileInputType == OpenVRProfileInputType::JOYSTICK;
}
    ...
    if (subMode == SubMode::CLICK)
    {
        switch (clickForceInput)
        {
            default:
                ...
                if (findComponent("force", OpenVRInputType::FLOAT, availableComponents) != -1 && click.tryGrab(ClickFilter::Source::BASE, ClickFilter::DefaultThresholdsType::CLICK, clickActivateThreshold, clickDeactivateThreshold, availableComponents, indexes)) break;
                if (findComponent("force", OpenVRInputType::FLOAT, availableComponents) != -1 && click.tryGrab(ClickFilter::Source::VALUE, ClickFilter::DefaultThresholdsType::CLICK, clickActivateThreshold, clickDeactivateThreshold, availableComponents, indexes)) break;
            ...
            case ForceInput::POSITION:
                if (click.tryGrab(ClickFilter::Source::CLICK, ClickFilter::DefaultThresholdsType::CLICK, clickActivateThreshold, clickDeactivateThreshold, availableComponents, indexes)) break;
                if (findComponent("force", OpenVRInputType::FLOAT, availableComponents) != -1 && click.tryGrab(ClickFilter::Source::BASE, ClickFilter::DefaultThresholdsType::CLICK, clickActivateThreshold, clickDeactivateThreshold, availableComponents, indexes)) break;
                if (findComponent("force", OpenVRInputType::FLOAT, availableComponents) != -1 && click.tryGrab(ClickFilter::Source::POSITION, ClickFilter::DefaultThresholdsType::CLICK, clickActivateThreshold, clickDeactivateThreshold, availableComponents, indexes)) break;
                if (click.tryGrab(ClickFilter::Source::FORCE, ClickFilter::DefaultThresholdsType::CLICK, clickActivateThreshold, clickDeactivateThreshold, availableComponents, indexes)) break;
                forceInactive = true; indexes.clear(); return indexes;
            case ForceInput::FORCE:
                if (click.tryGrab(ClickFilter::Source::CLICK, ClickFilter::DefaultThresholdsType::CLICK, clickActivateThreshold, clickDeactivateThreshold, availableComponents, indexes)) break;
...
std::vector<int> ToggleButtonMode::connect(const std::vector<Mode::ComponentDescription>& availableComponents, const bool* connectedOutputs)
{
    ...
    enableTouchOutput = findComponent("touch", OpenVRInputType::BOOLEAN, availableComponents) != -1 || findComponent("value", OpenVRInputType::FLOAT, availableComponents) != -1 || (button.forceInput == ForceInput::POSITION && findComponent("x", OpenVRInputType::FLOAT, availableComponents) != -1 && findComponent("y", OpenVRInputType::FLOAT, availableComponents) != -1);
...
{
    std::vector<int> indexes;
    indexes.push_back(findComponent("force", OpenVRInputType::FLOAT, availableComponents));
    return indexes;
}
