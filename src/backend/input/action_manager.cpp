...
bool ActionManager::loadBinding(const std::string& filePath, InputProfile* inputProfile)
{
        ...
        for (const auto& it: j.at("bindings").items())
        {
            ...
            if (it.value().contains("sources"))
            {
                ...
                for (const auto& source: it.value().at("sources"))
                {
                    ...
                    {
                        const OpenVRInputDescription& inputDescription = inputProfile->getOpenVRInputs()[i];
                    ...
                    OpenVRProfileInputType profileInputType = OpenVRProfileInputType::NONE;
                    for (int i = 0; i < inputProfile->getOpenVRProfileInputsCount(); i++)
                    {
                        const OpenVRProfileInputDescription& inputDescription = inputProfile->getOpenVRProfileInputs()[i];
            ...
            if (it.value().contains("poses"))
            {
                for (const auto& source: it.value().at("poses"))
                {
                    ...
                    for (int i = 0; i < inputProfile->getOpenVRInputsCount(); i++)
                    {
                        if (inputProfile->getOpenVRInputs()[i].type == OpenVRInputType::POSE && inputProfile->getOpenVRInputs()[i].path == path)
                        {
                            openVRInputIndex = i;
                            break;
                        }
            ...
            if (it.value().contains("haptics"))
            {
                for (const auto& haptic: it.value().at("haptics"))
                {
                    ...
                    for (int i = 0; i < inputProfile->getOpenVRInputsCount(); i++)
                    {
                        if (inputProfile->getOpenVRInputs()[i].type == OpenVRInputType::HAPTIC && inputProfile->getOpenVRInputs()[i].path == path)
                        {
                            openVRInputIndex = i;
                            break;
                        }
...
void ActionManager::loadDefaultBindingForControllerProfile(InputProfile* inputProfile)
{
...
static inline InputState convertOpenVRInputState(const OpenVRInputState& inputState, uint64_t inputSourceHandle)
{
    switch (inputState.type)
    {
        case OpenVRInputType::BOOLEAN:
            return InputState {
...
void ActionManager::update(const openvr::ActiveActionSet* activeActionSets, int actionSetSize, int activeActionSetsCount, const OpenVRInputState* inputStates, long currentTime)
{
    ...
    for (int i = 0; i < activeActionSetsCount; i++)
    {
        ...
        openvr::ActiveActionSet activeActionSetCopy {
    ...
    for (int priority = highestPriority; priority >= 0; priority--)
    {
        ...
        for (const openvr::ActiveActionSet& activeActionSet: activeActionSetsCopy)
        {
            ...
            int controllerFilter = activeActionSet.restrictToDevice != 0 ? (int) getDeviceFromInputSourcePath(pathHandleRegistry.getPath(activeActionSet.restrictToDevice), false) : 0;
            ...
            if (actionSet != nullptr)
            {
                for (ActionSet::Source& source: actionSet->sources)
                {
                    if (controllerFilter == 0 || (int) source.device == controllerFilter)
                    {
                ...
                for (auto& it: actionSet->actions)
                {
                    ...
                    for (Action::PoseSource& poseSource: action->poseSources)
                    {
                        if (controllerFilter == 0 || (int) poseSource.device == controllerFilter)
                        {
            ...
            if (secondaryActionSet != nullptr)
            {
                for (ActionSet::Source& source: actionSet->sources)
                {
                    if ((int) source.device != controllerFilter)
                    {
                        std::vector<InputState> collectedInputStates;
                        for (const ActionSet::Source::OpenVRInput& sourceInput: source.openVRInputs)
                        {
                ...
                for (auto& it: secondaryActionSet->actions)
                {
                    ...
                    for (Action::PoseSource& poseSource: action->poseSources)
                    {
                        if ((int) poseSource.device != controllerFilter)
                        {
                            if (!usedInputsCopy.contains(poseSource.openVRInputIndex))
                            {
                                poseSource.active = true;
